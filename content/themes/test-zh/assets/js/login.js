        ...
        success: function (data) {
          if (data.success) { //已登录、
            ...
            if (inviteCode != "") { //存在邀请码参数
              //查询邀请码对应的公司是否存在
              $.ajax({
                url: BASE_URL + INVITE_CODE_CHECK, type: 'POST', data: { inviteCode: inviteCode },
                success: function (data) {
                  if (data.success) { //邀请码正确
                    ...
                    if (window.USER_INFO.teamId != null) {//已经有团队了
                      ...
                      if (window.USER_INFO.teamId != nowTeam.id) {
                        //弹出是否加入新团队
                        self.showBlank();
                        self.showContainer($("#choseOne"));
                        self.loginDialog.showChoseOneDialog(nowTeam);
                      } else {
                        self.goToSystem();
                      }
                    } else {//没有团队
